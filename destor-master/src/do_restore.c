...
void* read_recipe_thread_gc() {
	...
	for (i=0; i < jcr.bv->number_of_files; i++) {
		...
		free_chunk(c);*/
 		//sync_queue_push(restore_recipe_queue, c);
 		printf("the j is %d\n",j );
		printf("r->chunknum is%d\n",r->chunknum);
		set_read_chunk_num_gc();
		for (j = 0; j < r->chunknum; j++) 
		{
			
			//TIMER_DECLARE(1);
			//TIMER_BEGIN(1);
			struct chunkPointer* cp = read_next_n_chunk_pointers_gc(jcr.bv, 1, &k);
			//printf("2");
			insert_word(bf,&cp->fp,sizeof(fingerprint));
			//printf("1");
			number_of_bf_fp++;

			//printf("%d ",number_of_bf_fp);
			//struct chunk* c = new_chunk(0);
			//memcpy(&c->fp, &cp->fp, sizeof(fingerprint));
			//c->size = cp->size;
			//c->id = cp->id;
...
//		sync_queue_push(restore_recipe_queue, c);
		//printf("e");
		printf("the end number_of_bf_fp  is%ld\n", number_of_bf_fp);
		free_file_recipe_meta(r);
	}
//	sync_queue_term(restore_recipe_queue);
	return NULL;
